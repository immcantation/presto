...
class TestIgCore(unittest.TestCase):
    ...
    def test_mergeAnnotation(self):
        result = presto.Annotation.mergeAnnotation(self.ann_dict_1, self.ann_dict_2)
        print result
        self.assertEqual('SEQ1', result['ID'])
        self.assertEqual('A,B,C,C', result['TEST1'])
        self.assertEqual('1,2,3', result['TEST2'])

        result = presto.Annotation.mergeAnnotation(self.ann_dict_1, self.ann_dict_2, prepend=True)
        print result
        self.assertEqual('SEQ1', result['ID'])
    ...
    def test_renameAnnotation(self):
        result = presto.Annotation.renameAnnotation(self.ann_dict_1, 'TEST1', 'RENAMED1')
        print result
        self.assertEqual('A,B', result['RENAMED1'])
        self.assertEqual(None, result.get('TEST1', None))

        result = presto.Annotation.renameAnnotation(self.ann_dict_1, 'TEST1', 'RENAMED2')
        print result
    ...
    def test_collapseAnnotation(self):
        result = presto.Annotation.collapseAnnotation(self.ann_dict_1, 'first')
        print result
        self.assertEqual('SEQ1', result['ID'])
        self.assertEqual('A', result['TEST1'])
        self.assertEqual(1, result['TEST2'])

        result = presto.Annotation.collapseAnnotation(self.ann_dict_1, 'last')
        print result
        self.assertEqual('SEQ1', result['ID'])
        self.assertEqual('B', result['TEST1'])
        self.assertEqual(2, result['TEST2'])

        result = presto.Annotation.collapseAnnotation(self.ann_dict_1, 'set')
        self.assertEqual('SEQ1', result['ID'])
        self.assertEqual(['A' ,'B'], result['TEST1'])
        self.assertEqual([1, 2], result['TEST2'])
        print result

        result = presto.Annotation.collapseAnnotation(self.ann_dict_1, 'cat')
        self.assertEqual('SEQ1', result['ID'])
        ...
        result = presto.Annotation.collapseAnnotation(self.ann_dict_1, 'min', fields='TEST2')
        self.assertEqual('1', result['TEST2'])
        print result

        result = presto.Annotation.collapseAnnotation(self.ann_dict_1, 'max', fields='TEST2')
        self.assertEqual('2', result['TEST2'])
        print result

        result = presto.Annotation.collapseAnnotation(self.ann_dict_1, 'sum', fields='TEST2')
